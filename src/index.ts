...
type DataHandler<T extends TSocketEndpointNames> = (
...
type SubscriptionMap<
  T extends TSocketSubscribableEndpointNames = TSocketSubscribableEndpointNames
> = Map<string, SubscriptionHandler<T>>;
...
export class DerivAPIClient {
  ...
  async send<T extends TSocketEndpointNames>({
    name,
    payload,
  }: SendFunctionArgs<T>) {
    ...
    if (matchingRequest)
      return matchingRequest.promise as Promise<TSocketResponseData<T>>;

    const { promise, resolve, reject } =
      PromiseUtils.createPromise<TSocketResponseData<T>>();
    ...
    this.requestHandler.set(
      this.req_id.toString(),
      newRequestHandler as RequestHandler<TSocketEndpointNames>
    );
  ...
  async subscribe<T extends TSocketSubscribableEndpointNames>({
    ...
    if (!matchingSubscription) {
      ...
      newSubscriptionHandler.subscriptions.set(
        newSubscriptionHandler.counter,
        onData
      );
      ...
      await this.waitForWebSocketOpen?.promise;
      this.websocket.send(
        JSON.stringify({ ...subscriptionPayload, req_id: this.req_id })
      );
      ...
      matchingSubscription.subscriptions.set(
        currentCounter,
        onData as (
          data: TSocketSubscribeResponseData<TSocketSubscribableEndpointNames>
        ) => void
      );
      matchingSubscription.counter = currentCounter;
      const matchingHandler =
        matchingSubscription.subscriptions.get(currentCounter);
  ...
  async unsubscribe({ hash, id }: UnsubscribeHandlerArgs) {
    ...
    const response = await this.send({
      name: "forget",
      payload: { forget: subscription_id },
    });
